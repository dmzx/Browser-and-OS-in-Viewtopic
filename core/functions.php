...
class functions
{
	...
	/**
	 * The name of the browser bot.	All return types are from the class contants
	 * @return string Name of the browser
	 */
	public function getName()
	{
		return $this->_name_name;
	}
	...
	public function reset()
	{
		...
		$this->_name_name = self::BROWSER_UNKNOWN;
	...
	/**
	 * Set the name of the browser bot
	 * @param $browser string The name of the Browser
	 */
	public function setName($name)
	{
		$this->_name_name = $name;
	}
	 ...
	 * @return boolean True if the browser is the Bot browser otherwise false
	 */
	protected function checkBrowserBot()
	{
		if (stripos(strtolower($this->_agent),'bot') !== false)
		{
			$aresult = explode('/', stristr($this->_agent, 'compatible'));
			preg_match('#compatible;(.*?);#', $this->_agent, $bot_detect);
			$aversion = explode(' ', isset($aresult[1]) ? $aresult[1] : "");
			$this->setVersion($aversion[0]);
			$this->setBrowser(self::BROWSER_UNKNOWN);
			$this->setName(isset($bot_detect[1]) ? $bot_detect[1] : "");
			return true;
		}
		return false;
	}
	...
	/**
	 * Determine if the user is using a Edge
	 * @return boolean True if the browser is the Edge browser otherwise false
	 */
	...
	protected function checkBrowserW3CValidator()
	{
		...
		else if (stripos($this->_agent, 'W3C_Validator') !== false)
		{
			...
			preg_match('/(W3C_Validator\/)([0-9]*).([0-9]{1,2})/', $this->_agent, $bot_detect);
			$aresult = explode('/', stristr($ua, 'W3C_Validator'));
			$aversion = explode(' ', $aresult[1]);
			$this->setVersion($aversion[0]);
			$this->_browser_name = self::BROWSER_UNKNOWN;
			$this->setName($bot_detect[1]);
			return true;
		}
